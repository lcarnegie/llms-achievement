...
As at 17 March 2025, the default OpenAI setting is that input data is used by default for training, unless otherwise specified in the ChatGPT settings. LC: It might be good to just be straightforward as to whether you had enabled this or not? Just so that you are consistent and systematic across analysis periods. Up to you though. @openai_data_usage_2023
...
If $y_i$ is the proportion (i.e. percentage mark) that student $i$ received on the final paper, then our model is:
...
<!-- Edited here  -->
Another approach for dealing with our particular situation (in which there are exact zeros or ones) could have been to slightly perturb the data (LC: by..? maybe like 5-10 words on this would help). In our case, that would likely have been defendable as there is not much difference between getting 99 per cent on a paper and getting 100 per cent. That said, when we estimated that perturbed data model using beta regression the results were similar to that of our one-inflated model. LC: And so why did we end up using the strategy we did? what made zero-inflated better? the fact that it accounted for those who got exactly 100% too? 
...
The first model in @tbl-model-results model data for STA302 and focus on the association between final paper mark and LLM usage. The next model controls for self-reported GPA. Both of these have 92 observations. This same pattern was then repeated for STA304, except with 131 new observations.  The third model for STA304 in @tbl-model-results additionally controls for whether the respondent is a native English speaker or not.
...
We can first take advantage of the simple initial models for STA302 and STA304, particularly the ZOI intercept, to get some simple insights. For instance, for the STA302 model it is -2.10. After undoing the logit transformation this is approximately 11 per cent, which corresponds to the number of students in STA302 who got full marks on the final paper [@andrewagain]. The same holds for the STA304 model, where -2.49 translates to about 8 per cent of STA304 students achieved full marks on the final paper.
...
There is some mild evidence from this observational study that LLM usage may be associated with better scores. However, this work should mostly be treated as a direction for future research in this area. This work underscores three broad takeaways: realizing the potential of LLMs in trustworthy data science will demand substantial advancement and integration; enhancing and fine-tuning chatbot and model expertise in data science could improve their utility; and exploring non-chatbot interfaces may offer better support for diverse learners, particularly those who are non-native English speakers.

## Impact of LLMs on trustworthy data science

We found that many students used LLMs to help them complete a data science project closely related to what professional data scientists do in industry(@aws_llm_explained). For instance, when student logs where inspected, a common use case was to ask the LLM to write code to make a graph, and another was to ask for some example paragraphs of text.

Data science code tends to be written differently than software engineering code. Functions and loops are used less, as there is often less reuse of code. The training data of these LLMs is dominated by software engineering code, and so when an LLM is asked to write code for a data science project it tends to produce code with some of those features. This is *prima facie* neither good nor bad in and of itself, however it can clash with the style of the code that surrounds it which could lead to maintainability difficulties. At the same time, recent developments in Retrival Augmented Generation (RAG), which enables Large Language Models such as GPT-4o to source text from across the web in real time [@Lewis2020RAG], has likely mitigated this discrepancy to some degree. Since models are now able to source relatively up-to-date documentation and knowledge on data science specific programming practices, the training data of the foundation models themselves has come to be less of a concern when querying for data science specific code. That said, this development has put a higher emphasis on the quality of the prompting methods used for these models in order to glean useful and working code to be used in trustworthy data science workflows. 

Again, looking at the logs, LLMs are also being used as an alternative search engine. Students often asked, for instance, how to modify a graph to add a vertical line, or similar. Here LLMs tended to be especially useful and able to answer questions. In the past students may have used Stack Overflow or similar to find their answers, but the nature of the chat response likely provided a faster answer. More recently, Retrival Augmented Generation capabilities, through features such as ChatGPT Search, have essentially allowed LLMs to become true search engines themselves, allowing for curated, personalized answers to programming and statistics questions at the stroke of a keyboard. While sources consulted by the LLMs are presented at the end of each query, it appears unlikely that students would spend the extensive time necessary to verify every single source for every single query, given that the sources in a single RAG generation can range from 1 to larger than 10. This could pose problems for attribution and verification of sources and solutions, particular in niche technical cases where a code problem could be solved with a simple addition of a R pipe (|>) or Python indent. 

Finally, there is the question of whether the nature of the data science has fundamentally changed or improved. For now, there is little evidence of that. It was clear (LC: when marking papers?) when a student had just used LLM-generated text without modification, and it was often clear when large amounts of code had been written by an LLM. But it was not obvious when small amounts of either had been used, or when LLM-generated content had just been used as an initial draft.
...
Retrieval-augmented generation (RAG) means providing various sources, for instance books or technical manuals, which the LLM uses to base its response on. Fine-tuning means providing the previously trained LLM with a large number of example input and outputs, which again guide the response from the LLM [@raffel2020exploring][@fan2024survey]. A chatbot-based interface, focused on the needs of data science beginners, could use RAG and/or fine-tuning to considerably improve the quality of its responses with respect to the topic of data science. The trade-off would be a reduction in the general-purpose usability of the chatbot, but in the context of this use, such a trade-off is likely acceptable. 

## Changed LLM-based interfaces

Applications such as WordTune [cite], had implemented practical use cases of Large Language Models such as OpenAI's GPT model series as early as 2020, but it was the launch of ChatGPT by OpenAI itself in 2022 that brought the technology into mainstream consciousness. While the underlying foundation model that OpenAI development was not considerably different in performance to others that were being developed at the time, the key innovation ChatGPT brought was an intuitive chat interaction model that allowed users to elicit answers from the models' knowledge bases in a conversational way. As of April 2025, the conversational modality of interaction remains the dominant form of interaction with Large Language Models, but there is considerably opportunity to develop additional ways of interacting with LLMs that are more tailored to distinct applications, like that of Data Science. This is particularly relevant for enhancing trustworthy data science, where reproducibility and documented integrity in the data-scientific process are important. 

The trustworthiness of  data science workflows could be considerably enhanced by establishing continuous integration of code functionality, data integrity, and the consistency between code, analysis, and reported results. Students in this class were required to use GitHub to host their papers, supporting code and data. GitHub Issues and Pull Requests were used for peer review and submission occurred through the provision of links to GitHub repositories by students. That said, their workflows could be made even better. In software engineering, continuous integration ensures that code quality and functionality during development, with automated tests ensuring that code is of high quality and functionality. Similarly, continuous integration of code functionality, data integrity, and the consistency between code, analysis, and reported results could be established. This could involve automated tests that verify data is loaded correctly, checks for reproducibility, static analysis to suggest improvements and catch inconsistencies, and even automated comparisons between what is written in the paper and what's actually happening in the code, a key area of strength for LLMs. Continuous integration for data science would bring accountability, repeatability, and interpretability — crucial for making it trustworthy. By developing automated systems of interaction that can facilite this continous integration, and teaching proper usage of them, the practice of trustworthy data science can be more effectively taught. 

On a conceptual front of trustworthy data science workflows, search-based tools have been making significant strides in improving the speed in the initial research phases of projects. Search-based LLMs tools such as Elicit, Perplexity.ai, increasingly ChatGPT and even institutional-supported tools like Scopus AI and WebOfScience differ to older Large Language Model systems in that they primarily focus on search and "answer generation". In constrast to Google, which provides a list of links related to a particular query, the previous tools attempt to provide summarized answers, with integrated web links to academic articles, websites, and other media for verification and further reading [@fan2024survey]. In particular, ChatGPT has increasingly focused on using RAG-based LLMs to create search engines, pushing up against Google's dominance of information retrieval on the internet. Using LLMs for information searching considerably changes the mental models of interaction with domain-specific knowledge as well as technical documentation. Particularly, the ontological focus of answer generation over the exploration of the data as it is risks creating rushed workflows, with poorly chosen models and visualizations that do not effectively make sense of datasets, creating problems for interpretability. To mitigate this, incoporating critical thinking at every step of the data science workflow is necessary, to ensure that proper statistical and programming methodologies are followed.

Another recent development with LLMs is that of "Agentic AI", enabled through tool use. At this point, most of the prominent research firms (OpenAI, Google DeepMind, Anthropic, xAI, etc.) have incorporated at least some autonomous (i.e agentic) capabilities within their chatbots. With recent advances in LLM-based "reasoning", such as OpenAI's o-series models [@openai2024o1], these tools are able to autonomously create strategies for data analysis, analyze datasets, compute and analyse basic statistics, choose and compute models and criticize their assumptions, significantly automating large swaths of the data science workflows. Tool use, focused on data science, also exists. For instance, LLMs can use R and Python to respond to queries about a dataset. For instance, when a user wrote code in an IDE to import some data, a tool-enhanced LLM could notice and automatically write code that establishes a Pydantic-based validation model for that data. Similarly, consider an LLM in an IDE context where a data scientist wrote out the model, in statistical notation, that they were interested in estimating, and the LLM was able write the model in Stan, run it, save the estimates, and add a summary table to the paper, all automatically and in the background. Similar work has occurred in other contexts and found to bring substantial benefits [@chen2022evaluating]. Similarly, the new Deep Research functinalities of Google DeepMind's Gemini [@google2024deepresearch], OpenAI's ChatGPT [@openai2025deepresearch] and xAI's Grok [@xai2025grok3] chatbots are able to autonomously browse the web, analyse sources, and present findings on prompted research questions. 

To ensure trustworthy and reproducible data science, the element of human interaction with the dataset must be considered in the evaluation of trustworthiness. If an entire analysis, while coherent, well-written, and reproducible, is done by an Agentic LLM, but in the name of a student, is that analysis actually trustworthy? Did the student actually learn how to create trustworthy and reproducible knowledge? Despite significant strides in reducing model hallucinations, LLMs still possess the demonstrated capability of making such mistakes, and these mistakes have been shown to be costly to those who lackadaisically put faith in the tools without critical thought of the outputs. Especially within data science workflows, which encompass many complex steps and areas where critically-informed verification of assumptions and results is key to success, creating LLM interfaces that allow for the verification of assumptions and outputs will become increasingly important. This is especially so as these autonomous tools become more engrained within the commonly-accepted data science workflows, reproducible or not. 
